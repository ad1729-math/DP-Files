...
def A(b, I ,n , g):
    ...
    for j in range(1, len(L1)):
        ...
        if x1==fr(x0,g):
            ...
            if Eo[Edges.index((y1, y0))][1]==1:
                t2+=1

        else: 
            if Eo[Edges.index((x0, fr(x0,g)))][1]==1:
                t1+=1
                if Eo[Edges.index((fr(x0,g) ,x1))][1]==1:
                    t1+=1
            elif Eo[Edges.index((fr(x0,g), x1))][1]==-1 and Eo[Edges.index((fr(x0,g),x0))][1]==1:
                    t1+=1
            else: 
                t1+=0 

            if Eo[Edges.index((y1, fl(y0,g)))][1]==1:
                t2+=1
                if Eo[Edges.index((fr(y0,g), y0))][1]==1:
                    t2+=1
            elif Eo[Edges.index((y1,fl(y0,g)))][1]==-1 and Eo[Edges.index((fl(y0,g),y0))][1]==1:
                    t2+=1
            else: 
                t2+=0 
    ...
    def Tor_dir1(i,j):
        ...
        if i in L1 and j in R1:
            if L1.index(i)==R1.index(j):
                return Dir1[L1.index(i)]

        elif i in R1 and j in L1: 
            if R1.index(i)==L1.index(j):
                return -Dir1[R1.index(i)]

        else: 
            return 0
        
    def Tor_dir2(i, j):
        if i in U1 and j in D1:
            if U1.index(i) == D1.index(j):
                return Dir2[U1.index(i)]

        elif i in D1 and j in U1:
            if D1.index(i) == U1.index(j):
                return -Dir2[D1.index(i)]
