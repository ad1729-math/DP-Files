...
def A(b, I ,n , g):
    ...
    L_half, R_half=[n0],[n0+1]  #Left and Right sides 
    i0,i1=n0,n0+1
    ...
    j0,j1=L_half[-1], R_half[-1]
    Ed1,Ed2=[(i0,i1)],[(j0,j1)]

    Dir1, Dir2=[], []

    if Eo[Edges.index((i0, i1))][1]==1:  Dir1.append(1)
    else: Dir1.append(-1)
    ...
    for k in range(1, len(L_half)-int(len(L_half)/2)):
        ...
        Ed2.append((L_half[j],R_half[j]))
        p0,q0=L_half[j],R_half[j]
        p1,q1=L_half[j-1],R_half[j-1]
        s1,s2=0,0
    ...
    def Tor_dir(i,j):
        ...
        if i in L_half and j in R_half: 
            v=L_half.index(i)
            if v<int(l/2) and L_half.index(i)==R_half.index(j):
                return Dir1[v]
            
            elif v>=int(l/2) and L_half.index(i)==R_half.index(j):
                return Dir2[l-v-1]
            ...
            v=R_half.index(i)
            if v<int(l/2) and R_half.index(i)==L_half.index(j):
                return -Dir1[v]
            
            elif v>=int(l/2) and R_half.index(i)==L_half.index(j):
                return -Dir2[l-v-1]
